...
const formFields = ['memberId', 'type', 'imageUrl', 'amount']
...
export default {
  ...
  watch: {
    visible(val) {
      if (val) {
        this.$nextTick(() => {
          const values = {}
          for (let key in this.memberShareDetail) {
            if (formFields.includes(key)) {
              values[key] = this.memberShareDetail[key]
            }
          }
          if (values.imageUrl) {
  ...
  methods: {
    ...
    onReset() {
      ...
      this.$emit('update:memberShareDetail', {})
    },
