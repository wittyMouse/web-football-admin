...
<script>
const formFields = ['dayNum', 'donateIntegral', 'type']
...
export default {
  ...
  watch: {
    visible(val) {
      if (val) {
        this.$nextTick(() => {
          const values = {}
          for (let key in this.checkInDetail) {
            if (formFields.includes(key)) {
              values[key] = this.checkInDetail[key]
            }
          }
  ...
  methods: {
    onReset() {
      ...
      this.$emit('update:checkInDetail', {})
    },
